...
This guide explains how to register your smart contract in the Qom One dApp store, and start earning income every time a user interacts with your smart contract. {synopsis}

The Qom One dApp store is a revenue-per-transaction model, which allows developers to get paid for deploying their decentralized application (dApps) on Qom One. Developers generate revenue every time a user interacts with their dApp in the dApp store, providing them a steady income. Users can discover new applications in the dApp store and pay for the transaction fees that finance the dApp's revenue. This value-reward exchange of dApp services for transaction fees is implemented by the [x/revenue module](../../../x/revenue/spec/01_concepts.md).
...
qomd tx revenue register $CONTRACT $NONCE $WITHDRAWER \
...
qomd tx revenue update $CONTRACT $WITHDRAWER \
...
qomd tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000aevmos \
--from=mm
```
