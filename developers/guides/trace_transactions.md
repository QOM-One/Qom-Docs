...
Learn how to trace transactions and blocks on Qom One. {synopsis}
...
Tracing a transaction means requesting an Qom One node to re-execute the desired transaction with varying degrees of data collection.
...
Qom One supports the following `debug_trace*` JSON-RPC Methods, which follow [Geth's debug API guidelines](https://geth.ethereum.org/docs/rpc/ns-debug).
...
- [JSON-RPC Methods on Qom One](../json-rpc/endpoints.md)
