...
In their Native Coin representation, they can be transferred between accounts on Qom One and other Cosmos chains using IBC.
They cannot, however, be used to interact with dApps on Qom One, as Native Coins are not supported by the EVM since they don’t implement the ERC-20 standard.

In order to reduce end-user complexity, Qom One should only allow single-token representation use between IBC Coin and ERC-20s.
Consequently, the Qom One team developed the Automated Coin Conversion feature to achieve this goal.
...
Your users may want to move their ERC-20 tokens from Qom One onto other Cosmos chains.
...
For example, if you want to transfer the ERC-20 representation of the `uosmo` token on Qom One,
...
Before transferring ERC-20 tokens via IBC, make sure you [register the ERC-20 token](https://docs.Qom One.org/developers/guides/erc20_registration.html) for the conversion.
...
:::tip
**Note**: In case Qom One is not the source chain of the sent IBC coin, you will have to specify the corresponding IBC denom (e.g. `ibc/ED07A3391A112B175915CD8FAF43A2DA8E4790EDE12566649D0C2F97716B8518`)
:::
...
Your users may want to move IBC Coins from other Cosmos chains onto Qom One.
To use these IBC coins on dApps deployed on Qom One, they need an ERC-20 representation of these.
...
:::tip
**Note**: If your users have IBC coins on Qom One already, and they receive an IBC transfer in the denomination of an already registered token pair,
...
For example, if we want to send an ERC-20 token called `TestCoin` via IBC, use `Token.Denom = "erc20/<test-coin-contract-address>"`.
Keep in mind that to perform this operation, you need to [register the token pair](https://docs.Qom One.org/developers/guides/erc20_registration.html) previously.

### Can I send WQom One to other chains?

WQom One transfers are not supported at the moment.
However, you can unwrap manually the WQom One tokens using the [Qom One dashboard](https://app.Qom One.org/assets) or [Diffusion](https://app.diffusion.fi/).
Then you can perform a regular IBC transfer using the Qom One tokens.

### Does automated coin conversion apply to all coins?

The automated coin conversion covers all IBC coins and ERC-20 tokens as long as the appropriate token mapping was registered through governance ([guide to register an ERC-20 token](https://docs.Qom One.org/developers/guides/erc20_registration.html)).
If the token pair is not registered, the IBC coin will be left as is.
Additionally, Qom One token conversion is not automated.
Considering that the Qom One token is used for staking and paying gas fees, the team decided to exclude the native token automated conversion.
Thus, the user experience is not undermined by this feature.

### How do I convert the Qom One token to ERC-20?

The conversion from Qom One token to WQom One is not automated.
If you want to convert Qom One tokens into its ERC-20 representation, you will need to use [the Assets page](https://app.Qom One.org/assets).

### Do I still need to use [the Assets page](https://app.Qom One.org/assets)?

Yes! If you want to convert Qom One tokens into their ERC-20 representation, you will need to do it manually on [the Assets page](https://app.Qom One.org/assets).
Qom One token automated conversion was excluded in this feature because it is used for staking, governance and paying for gas on the EVM.
