...
The Qom One supports different clients in order to support Cosmos and Ethereum transactions
and queries:

|                                                        | Description                                                                          | Default Port |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------ | ------------ |
| **Cosmos [gRPC](#cosmos-grpc)**                        | Query or send Qom One transactions using gRPC                                          | `9090`       |
| **Cosmos REST ([gRPC-Gateway](#cosmos-grpc-gateway))** | Query or send Qom One transactions using an HTTP RESTful API                           | `9091`       |
...
| **Command Line Interface ([CLI](#cli))**               | Query or send Qom One transactions using your Terminal or Console.                     | N/A          |

## Cosmos gRPC

Qom One exposes gRPC endpoints for all the integrated Cosmos SDK modules. This makes it easier for
...
See the list of supported gRPC-Gateway API endpoints for the Qom One testnet [here](https://api.Qom One.dev/).

## Ethereum JSON-RPC

<!-- TODO: Link JSON-RPC docs -->

Qom One supports most of the standard [JSON-RPC APIs](./json-rpc/server.md) to connect with existing Ethereum-compatible web3 tooling.
...
Check the `Events` page to obtain the event list of each supported module on Qom One.
:::
...
Users can use the `{{ $themeConfig.project.binary }}` binary to interact directly with an Qom One node
though the CLI.
