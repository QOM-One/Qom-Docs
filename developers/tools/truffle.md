...
This will allow your contract to connect to your Qom One local node.
...
You should see incoming deployment logs in the Qom One daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Qom One node using the `test` command:
