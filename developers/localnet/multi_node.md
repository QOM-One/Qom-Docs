...
This command creates a 4-node network using the `qomdnode` Docker image.
...
Creating qomdnode0 ... done
Creating qomdnode2 ... done
Creating qomdnode1 ... done
Creating qomdnode3 ... done
```
...
calling the `qomd testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/qomd` directory in each container.
...
```bash
# node 0: daemon logs
docker exec qomdnode0 tail qomd.log
...
To interact with `qomd` and start querying state or creating txs, you use the
`qomd` directory of any given node as your `home`, for example:

```bash
qomd keys list --home ./build/node0/qomd
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/qomd/key_seed.json` and can be restored to the CLI using the `qomd keys add --restore` command
:::
