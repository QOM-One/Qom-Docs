...
The `qomd debug addr <address>` can be used to convert an address between hex and bech32 formats. For example:
...
::: tip
The Cosmos SDK Keyring output (i.e `qomd keys`) only supports addresses and public keys in Bech32 format.
:::

We can use the `keys show` command of `qomd` with the flag `--bech <type> (acc|val|cons)` to
...
qomd q auth account $(qomd keys show dev0 -a) -o text
