...
Learn how to run an IBC Relayer for Qom. {synopsis}
...
## Prerequisites
<!-- textlint-disable -->
Before beginning, ensure you have an Qom node running in the background of the same machine that you intend to relay on. Follow [this guide](quickstart/run_node.md) to set up an Qom node if you have not already.
<!-- textlint-enable -->

In this guide, we will be relaying between [Qom (channel-3) and Cosmos Hub (channel-292)](https://www.mintscan.io/qom/relayers). When setting up your Qom and Cosmos full nodes, be sure to offset the ports being used in both the `app.toml` and `config.toml` files of the respective chains (this process will be shown below).

<!-- textlint-disable -->
In this example, the default ports for Qom will be used, and the ports of the Cosmos Hub node will be manually changed.
<!-- textlint-enable -->

## Qom Daemon Settings

First, set `grpc server` on port `9090` in the `app.toml` file from the `$HOME/.qomd/config` directory:

```bash
vim $HOME/.qomd/config/app.toml
```
...
Then, set the `pprof_laddr` to port `6060`, `rpc laddr` to port `26657`, and `prp laddr` to `26656` in the `config.toml` file from the `$HOME/.qomd/config` directory:

```bash
vim $HOME/.qomd/config/config.toml
```
...
```bash
# In this example, we will set channel-292 on the cosmoshub-4 chain settings and channel-3 on the qom_7668378 chain settings:
[[chains]]
id = 'cosmoshub-4'
...
policy = 'allow'
list = [
   ['transfer', 'channel-292'], # qom_7668378
]
...
hermes keys restore qom_7668378 -m "24-word mnemonic seed"
```

Ensure this wallet has funds in both Qom and ATOM in order to pay the fees required to relay.
...
INFO ThreadId(01) [qom_7668378] chain is healthy
```
...
```bash
hermes query packet unreceived-packets qom_7668378 transfer channel-3
hermes query packet unreceived-acks qom_7668378 transfer channel-3
```
...
hermes query packet commitments qom_7668378 transfer channel-3
```
...
hermes clear packets qom_7668378 transfer channel-3
```
...
```bash
hermes tx raw packet-recv qom_7668378 cosmoshub-4 transfer channel-292
hermes tx raw packet-ack qom_7668378 cosmoshub-4 transfer channel-292
hermes tx raw packet-recv cosmoshub-4 qom_7668378 transfer channel-3
hermes tx raw packet-ack cosmoshub-4 qom_7668378 transfer channel-3
```
