...
Find below a list of `qomd` commands added with the `x/vesting` module. You can obtain the full list by using the `qomd -h` command.
...
```go
qomd add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
qomd query vesting balances ADDRESS [flags]
```
...
```go
qomd tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
qomd tx vesting clawback ADDRESS [flags]
```
...
```go
qomd tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
